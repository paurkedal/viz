...
open Prereq
open Unicode
module UCharInfo = CamomileLibraryDefault.Camomile.UCharInfo
open Diag
open Cst_types
open Cst_core
open Leaf_types
...
let initial_conditionals = [
  ...
  "if",        Grammar.IF;
  "else",      Grammar.ELSE;
  "taken",     Grammar.COND (Idr "taken");
  "when",      Grammar.COND (Idr "when");
]
...
let scan_lexalias state lex_loc =
  ...
  | op_pairs, None ->
      ...
      let loc = Textloc.between (Textloc.lbound lex_loc)
