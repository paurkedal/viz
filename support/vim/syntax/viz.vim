...
let s:pattern_conditionals =
  \ ['at']
let s:declarators =
  \ ['in', 'inj', 'include', 'let', 'lex', 'open', 'sig', 'type', 'use', 'val']
let s:connectives =
...
syn match VizSctExprStart contained '\K\k*' skipwhite nextgroup=VizSctExprCont
syn match VizSctExprCont contained '\.\K\k*' skipwhite nextgroup=VizSctExprCont
syn region VizSctExprCont contained fold transparent matchgroup=VizPathOperator
  \ start='\.(' end=')' contains=@VizSctExpr
  \ skipwhite nextgroup=VizSctExprCont
...
"
syn cluster VizSigExpr contains=VizSigName,VizSigProduct,VizSigPath,VizSigPrefix
...
syn match VizCommentHeader contained '\(^#\s\+\)\@<=Z\.\([0-9Z]\+\.\)*\s.*'
...
hi def link VizCommentHeader CommentHeader
